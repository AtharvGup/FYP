...
def _threshold_and_support_graph(gidx:HeteroGraphIndex, scores:Tensor, end_n_ids:Tensor):
    ...
    in_degrees = gspmm(gidx, "copy_rhs", "sum", None, torch.ones_like(scores))[0]
    ...
    support_size = gspmm(gidx, "copy_rhs", "sum", None, support.float())[0]
...
class EdgeSparsemaxFunction(Function):
    ...
    def forward(ctx, gidx:HeteroGraphIndex, scores:Tensor,
        ...
        scores = scores.float()
        scores_max = gspmm(gidx, "copy_rhs", "max", None, scores)[0]
        scores = gsddmm(gidx, "sub", scores, scores_max, "e", "v")
        ...
        out = torch.clamp(gsddmm(gidx, "sub", scores, tau, "e", "v"), min=0)
    ...
    def backward(ctx, grad_out):
        ...
        v_hat = gspmm(gidx, "copy_rhs", "sum", None, grad_in)[0] / supp_size.to(out.dtype)
        grad_in_modify = gsddmm(gidx, "sub", grad_in, v_hat, "e", "v")
